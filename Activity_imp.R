...
# Read in data 
raw_data <- read_excel("C:/Users/malin/OneDrive - University of Bergen/MASTER  i Utvifysio/Publisering/Data/Behaviour_data/Raw_data.xlsx")
View(raw_data)



####################################################################################################################################
################## Define acclimation, filter time metric remove outliers, calculate averages #####################################
...
long_data <- raw_data|>
  ...
  mutate(Trial = substr(Variable, 1, 1),
         Metric = ifelse(grepl("distance", Variable), "Distance", "Time"),
...
# Filter data to only include the Time metric
long_data_no_outliers_time <- long_data_no_outliers|>
  filter(Metric == "Time")

# Update the Trial factor levels
long_data_no_outliers_time$Trial <- factor(long_data_no_outliers_time$Trial, levels = c("N", "H", "R"))

# Filter data to only include the distance metric
long_data_no_outliers_distance <- long_data_no_outliers|>
  filter(Metric == "Distance")

# Update the Trial factor levels
long_data_no_outliers_distance$Trial <- factor(long_data_no_outliers_distance$Trial, levels = c("N", "H", "R"))
...
hypoxia_plot <- ggplot(long_data_no_outliers_time[long_data_no_outliers_time$Acclimation == "Acclimated", ],
  ...
  labs(title = "B: Fish Activity Levels (Acclimated)", x = "Treatment", y = "", shape = "Sex", color = "Sex", fill = "Treatment") +
  theme_minimal() +
...
normoxia_plot <- ggplot(long_data_no_outliers_time[long_data_no_outliers_time$Acclimation == "Not Acclimated", ],
                        aes(x = Trial, y = Value_no_outliers, shape = Sex, fill = Trial)) +
  geom_boxplot(aes(group = Trial), outlier.shape = NA, position = position_dodge(0.8), color = "black") +
  geom_jitter(aes(color = Sex), position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.8), size = 1.5) +
  scale_color_manual(values = c("black", "black")) +
  ...
  scale_shape_manual(values = c("M" = 16, "F" = 17)) +
  labs(title = "A: Fish Activity Levels (Not Acclimated)", x = "Treatment", y = "Time spent moving", color = "Sex", fill = "Treatment") +
  theme_minimal() +
  scale_y_continuous(expand = expand_scale(add = c(0, 0.05)), limits = c(0, 160), breaks = seq(0, 160, 20)) +
...
correlation_data <- long_data|>
  filter(Metric == "Time")|>
  dplyr::select(Fish_ID, Trial, Value)|>
  rename(Time_Value = Value)|>
  left_join(long_data|>
              filter(Metric == "Distance")|>
              dplyr::select(Fish_ID, Trial, Value)|>
              rename(Distance_Value = Value))
...
length_weight_correlation <- cor.test(raw_data$Length_finished, raw_data$Weight_finished)
...
length_weight_plot <- ggplot(raw_data, aes(x = Length_finished, y = Weight_finished)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
